...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($stmt->execute()) {
            ...
            // Guardar solicitudes de laboratorio si existen
            if (isset($_POST['laboratorios']) && is_array($_POST['laboratorios'])) {
                $solicitudStmt = $conn->prepare("
                    INSERT INTO solicitudes_laboratorio (id_historial, id_paciente, id_tipo_laboratorio, observaciones) 
                    VALUES (?, ?, ?, ?)
                ");
                
                $laboratoriosCount = 0;
                foreach ($_POST['laboratorios'] as $idLaboratorio) {
                    if (!empty($idLaboratorio)) {
                        $solicitudStmt->execute([
                            $historial_id,
                            $_POST['id_paciente'],
                            $idLaboratorio,
                            "Solicitado desde consulta médica - " . $_POST['motivo_consulta']
                        ]);
                        $laboratoriosCount++;
                    }
                }
                ...
                if ($laboratoriosCount > 0) {
                    $_SESSION['message'] .= " y se han solicitado " . $laboratoriosCount . " laboratorio(s)";
                }
            }
            ...
            if (!empty($proxima_cita)) {
                ...
                } if (!empty($_POST['laboratorios'])) {
                    $labNames = [];
                    $labStmt = $conn->prepare("SELECT nombre FROM tipos_laboratorio WHERE id_tipo_laboratorio = ?");
                    foreach ($_POST['laboratorios'] as $labId) {
                        $labStmt->execute([$labId]);
                        $lab = $labStmt->fetch(PDO::FETCH_ASSOC);
                        if ($lab) $labNames[] = $lab['nombre'];
                    }
                    $examenesRealizados = implode(', ', $labNames);

                    // Actualizar el registro médico con los nombres
                    $updateStmt = $conn->prepare("UPDATE historial_clinico SET examenes_realizados = ? WHERE id_historial = ?");
                    $updateStmt->execute([$examenesRealizados, $historial_id]);
                } else {
