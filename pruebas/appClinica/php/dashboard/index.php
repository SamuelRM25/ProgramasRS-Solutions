...
try {
    ...
    if ($userType === 'admin') {
        // [Todo el código de admin que ya tienes - mantener igual]
        
        // Obtener el conteo de citas para hoy
        $today = date('Y-m-d');
        ...
        // Obtener el conteo de pacientes solo para el año actual
        $current_year = date('Y');
        ...
        // Obtener el conteo de citas pendientes (todas las citas futuras)
        ...
        // Obtener las consultas de este mes (todas las citas del mes actual)
        ...
        // Obtener las citas programadas para hoy con detalles
        $stmt = $conn->prepare("
        ...
        // Limpiar variables que no usarán otros usuarios
        $total_citas = 0;
        ...
        // Para doctores, mostrar solo sus citas
        $today = date('Y-m-d');
        
        // Obtener citas del doctor para hoy - CORREGIDO
        $stmt = $conn->prepare("
            SELECT c.id_cita, c.nombre_pac, c.apellido_pac, c.hora_cita, c.telefono 
            FROM citas c
            WHERE c.fecha_cita = ? AND c.id_medico = ?
            ORDER BY c.hora_cita
        ");
        $stmt->execute([$today, $userId]);
        $todays_appointments = $stmt->fetchAll(PDO::FETCH_ASSOC);
        
        // Obtener total de citas del doctor - CORREGIDO
        $stmt = $conn->prepare("SELECT COUNT(*) as total_citas FROM citas WHERE id_medico = ?");
        $stmt->execute([$userId]);
        $total_citas = $stmt->fetch(PDO::FETCH_ASSOC)['total_citas'] ?? 0;
        
        // Obtener pacientes del doctor - CORREGIDO
        $stmt = $conn->prepare("
            SELECT COUNT(DISTINCT CONCAT(c.nombre_pac, ' ', c.apellido_pac)) as total_patients 
            FROM citas c
            WHERE c.id_medico = ?
        ");
        $stmt->execute([$userId]);
        ...
        // Limpiar variables que no usará el doctor
        $today_appointments = count($todays_appointments);
        ...
        // Obtener citas de hoy (sin filtrar por médico para que puedan ver todas)
        $stmt = $conn->prepare("
            SELECT id_cita, nombre_pac, apellido_pac, hora_cita, telefono 
            FROM citas 
            WHERE fecha_cita = ?
        ...
        // Limpiar variables que no usará el usuario normal
        $today_appointments = count($todays_appointments);
        $total_patients = 0;
        $pending_appointments = 0;
        $month_consultations = 0;
...
<div class="d-flex">
    ...
    <div class="sidebar p-3">
        <div class="d-flex align-items-center mb-3 mb-md-0 me-md-auto">
            <span class="fs-4">Centro Médico de Pruebas</span>
        </div>
        <hr>
        <ul class="nav nav-pills flex-column mb-auto">
            <!-- Dashboard siempre visible para todos los usuarios -->
            <li class="nav-item">
            ...
            <?php
            if ($_SESSION['tipoUsuario'] === 'admin' || $_SESSION['tipoUsuario'] === 'user' || $_SESSION['tipoUsuario'] === 'doc') {
                // Agregar Enlace a Encamamiento
                ?>
                <li>
                    <a href="../encamamiento/index.php" class="nav-link text-dark">
            ...
            <?php if ($_SESSION['tipoUsuario'] === 'admin' || $_SESSION['tipoUsuario'] === 'user' || $_SESSION['tipoUsuario'] === 'farmacia'): ?>
            <!-- Módulos para admin, user y farmacia -->
            <li>
            ...
            <!-- Módulo de Pacientes visible para admin, doc y user -->
            <li>
            ...
            <?php if ($_SESSION['tipoUsuario'] === 'admin' || $_SESSION['tipoUsuario'] === 'doc' || $_SESSION['tipoUsuario'] === 'user'): ?>
            <!-- Módulo de Laboratorios visible para admin, doc y user -->
            <li>
                <a href="../laboratories/index.php" class="nav-link text-dark">
            ...
            <?php if ($_SESSION['tipoUsuario'] === 'admin' || $_SESSION['tipoUsuario'] === 'user'): ?>
                <li>
                    <a href="../minor_procedures/index.php" class="nav-link text-dark">
                        <i class="bi bi-bandaid me-2"></i>
                        Procedimientos Menores
                    </a>
                </li>
                <li>
                    <a href="../examinations/index.php" class="nav-link text-dark">
            ...
            <li>
                <a href="../reports/index.php" class="nav-link text-dark">
            ...
            <?php if ($_SESSION['tipoUsuario'] === 'admin' || $_SESSION['tipoUsuario'] === 'user' || $_SESSION['tipoUsuario'] === 'farmacia'): ?>
            <!-- Módulo de Cobros para admin, user y farmacia -->
            <li>
            ...
            <!-- Módulo de Traslados para admin y farmacia -->
            <?php if ($_SESSION['tipoUsuario'] === 'admin' || $_SESSION['tipoUsuario'] === 'farmacia'): ?>
            <li>
                <a href="../transfers/index.php" class="nav-link text-dark">
    ...
    <div class="main-content flex-grow-1">
        <div class="container-fluid">
                ...
                <div class="col-md-6 col-xl-4">
                    <div class="card">
                        <div class="card-body">
                            <div class="d-flex align-items-center">
                                <div class="flex-grow-1">
                                    <h6 class="text-muted mb-2">Mis Citas de Hoy</h6>
                                    <h4 class="mb-0"><?php echo $today_appointments; ?></h4>
                                </div>
                                <div class="avatar bg-light-primary">
                                    <i class="bi bi-calendar-check fs-3 text-primary"></i>
                ...
                <div class="col-md-6 col-xl-4">
                    <div class="card">
                        <div class="card-body">
                            <div class="d-flex align-items-center">
                                <div class="flex-grow-1">
                                    <h6 class="text-muted mb-2">Total de Mis Citas</h6>
                ...
                <div class="col-md-6 col-xl-4">
                    <div class="card">
                        <div class="card-body">
                            <div class="d-flex align-items-center">
                                <div class="flex-grow-1">
                                    <h6 class="text-muted mb-2">Mis Pacientes</h6>
                                    <h4 class="mb-0"><?php echo $total_patients; ?></h4>
                                </div>
                                <div class="avatar bg-light-info">
                ...
                <div class="col-md-6 col-xl-6">
                    <div class="card">
                        <div class="card-body">
                            <div class="d-flex align-items-center">
                                <div class="flex-grow-1">
                                    <h6 class="text-muted mb-2">Bienvenido al Sistema</h6>
                                    <p class="mb-0">Sistema de Gestión Clínica</p>
                                </div>
                                <div class="avatar bg-light-success">
            ...
            <?php
            // Verificar si el usuario es admin, servicio o doctor
            if ($_SESSION['tipoUsuario'] === 'admin' || $_SESSION['tipoUsuario'] === 'user' || $_SESSION['tipoUsuario'] === 'doc') {
                // Obtener camas ocupadas
                $stmt = $conn->query("
                    SELECT c.id_cama, c.numero, p.nombre, p.apellido, p.fecha_nacimiento,
                        m.nom_medicamento, m.cantidad, m.dosis, s.pa, s.fc, s.fr, s.glucosa, s.temperatura
                    FROM camas c
                    JOIN encamamiento_ingresos ei ON ei.id_cama = c.id_cama AND ei.estado = 'Activo'
                    JOIN pacientes p ON p.id_paciente = ei.id_paciente
                    LEFT JOIN (
                        SELECT id_ingreso, nom_medicamento, SUM(cantidad) AS cantidad, dosis
                        FROM encamamiento_medicamentos
                        JOIN inventario ON encamamiento_medicamentos.id_inventario = inventario.id_inventario
                        GROUP BY id_ingreso, nom_medicamento, dosis
                    ) m ON m.id_ingreso = ei.id_ingreso
                    LEFT JOIN (
                        SELECT id_ingreso, pa, fc, fr, glucosa, temperatura
                        FROM encamamiento_signos
                        ORDER BY fecha_hora DESC
                    ) s ON s.id_ingreso = ei.id_ingreso
                    WHERE c.estado = 'Ocupada'
                    GROUP BY c.id_cama
                ");
                $camasOcupadas = $stmt->fetchAll(PDO::FETCH_ASSOC);

                // Mostrar camas ocupadas
                ?>
                <div class="row mt-4">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header bg-secondary text-white">
                                <h5 class="mb-0"><i class="bi bi-bed me-2"></i>Camas Ocupadas</h5>
                            </div>
                            <div class="card-body">
                                <?php if (count($camasOcupadas) > 0): ?>
                                    <div class="table-responsive">
                                        <table class="table table-hover">
                                            ...
                                            <tbody>
                                                <?php foreach ($camasOcupadas as $cama): ?>
                                                    <tr>
                                                        <td><?php echo $cama['numero']; ?></td>
                                                        <td><?php echo $cama['nombre'] . ' ' . $cama['apellido']; ?></td>
                                                        <td><?php echo (new DateTime())->diff(new DateTime($cama['fecha_nacimiento']))->y; ?></td>
                                                        <td><?php echo $cama['nom_medicamento'] ?? 'N/A'; ?></td>
                                                        <td><?php echo $cama['cantidad'] ?? 'N/A'; ?></td>
                                                        <td><?php echo $cama['dosis'] ?? 'N/A'; ?></td>
                                                        <td><?php echo $cama['pa'] ?? 'N/A'; ?></td>
                                                        <td><?php echo $cama['fc'] ?? 'N/A'; ?></td>
                                                        <td><?php echo $cama['fr'] ?? 'N/A'; ?></td>
                                                        <td><?php echo $cama['glucosa'] ?? 'N/A'; ?></td>
                                                        <td><?php echo $cama['temperatura'] ?? 'N/A'; ?></td>
                                                    </tr>
                                                    <div class="modal fade" id="modalCama<?php echo $cama['id_cama']; ?>" tabindex="-1">
                                                        <div class="modal-dialog">
                                                            <div class="modal-content">
                                                                <div class="modal-header">
                                                                    <h5 class="modal-title">Cama <?php echo $cama['numero']; ?> - Detalles</h5>
                                                                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                                                </div>
                                                                <div class="modal-body">
                                                                    <p><strong>Paciente:</strong> <?php echo $cama['nombre'] . ' ' . $cama['apellido']; ?></p>
                                                                    <p><strong>Edad:</strong> <?php echo (new DateTime())->diff(new DateTime($cama['fecha_nacimiento']))->y; ?></p>
                                                                    <p><strong>Medicamento:</strong> <?php echo $cama['nom_medicamento'] ?? 'N/A'; ?></p>
                                                                    <p><strong>Cantidad:</strong> <?php echo $cama['cantidad'] ?? 'N/A'; ?></p>
                                                                    <p><strong>Dosis:</strong> <?php echo $cama['dosis'] ?? 'N/A'; ?></p>
                                                                    <p><strong>PA:</strong> <?php echo $cama['pa'] ?? 'N/A'; ?></p>
                                                                    <p><strong>FC:</strong> <?php echo $cama['fc'] ?? 'N/A'; ?></p>
                                                                    <p><strong>FR:</strong> <?php echo $cama['fr'] ?? 'N/A'; ?></p>
                                                                    <p><strong>Glucosa:</strong> <?php echo $cama['glucosa'] ?? 'N/A'; ?></p>
                                                                    <p><strong>Temp:</strong> <?php echo $cama['temperatura'] ?? 'N/A'; ?></p>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                ...
                                <?php else: ?>
                                    <div class="alert alert-info mb-0">
                                        <i class="bi bi-info-circle me-2"></i>No hay camas ocupadas.
                                    </div>
                                <?php endif; ?>
                            </div>
                        </div>
            ...
            <?php if ($userType === 'admin'): ?>
            <!-- Medicamentos por Caducar - Solo para Admin -->
            <div class="row mt-4">
                <div class="col-12">
                    <div class="card">
            ...
            <!-- Medicamentos con Stock Bajo - Solo para Admin -->
